...
- **Multi-caption training pipeline** — `tools/build_captions_to_metadata.py` builds a `captions.json` with multiple caption variants per image. Adapted from [generate-captions.py](https://github.com/motimalu/diffusion-training-configs/blob/main/diffusion-pipe/anima/utils/generate-captions.py) by [motimalu](https://github.com/motimalu).
